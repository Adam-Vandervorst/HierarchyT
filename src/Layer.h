...
public:
    ...
    Layer() = default;
    explicit Layer(Layer* parent, std::string name) : parent(parent), name(std::move(name)) { level = parent->level + 1;}

    Layer* add_layer(std::string new_name = "") {
        auto new_layer = new Layer(this, (new_name == "") ? name + "_" + std::to_string(children.size()) : new_name);
        children.push_back(new_layer);
        return new_layer;
    }

    template <typename DataType>
    std::tuple<Address> wrap(DataType item) {
        data.push_back(item);
        return std::make_tuple(std::make_pair(data.size() - 1, level));
    }
    ...
    auto get_objects(Address s, Address p) {
        auto [begin, end] = conn.equal_range(s);
    ...
    Address hylo(Address seed, F coalg, F alg) {
        this->data[seed.first] = coalg(this, seed);
        ...
        std::for_each(temp.begin(), temp.end(), [&](Address& o) {(it++)->second.second = hylo(o, coalg, alg);});
        this->data[seed.first] = alg(this, seed);
        return seed;
    }
};
